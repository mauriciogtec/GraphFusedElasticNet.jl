...
function intersection(l1::Line, l2::Line)
    @assert l1.slope != l2.slope "slopes should be different but are $(l1.slope) and $(l2.slope)"
...
struct RejectionSampler
    ...
    RejectionSampler(
        ...
        @assert a1 >= 00 "logf must have positive slope at initial cutpoint 1 but was $(a1)"
        @assert a2 <= 0.0 "logf must have negative slope at initial cutpoint 2 but was $(a2)"
    ...
    RejectionSampler(
        ...
        RejectionSampler(f, support, (x1, x2); from_log=from_log, kwargs...)
    end
end
